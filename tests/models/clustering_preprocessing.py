...
class ClusteringPreprocess:
	...
	def LOB_preprocessing(self, data, asset_char):
		...
		data[['liquidity_ref','smoothed_size','max_local']] = 0.0
		...
		for _, chunk in tqdm(data.groupby('index'), desc='Preprocessing data', total=len(data['index'].unique()), ncols=100, mininterval=60):
			...
			for _, c in chunk.groupby('side'):
				...
				chunk['max_local'] = chunk['smoothed_size'].rolling(window=5, center=True).max()
				chunk['max_local'] = chunk.apply(lambda row: True if row['max_local'] == row['smoothed_size'] else False, axis=1)
			...
			data.loc[chunk.index, ['side', 'liquidity_ref', 'smoothed_size', 'max_local']] = chunk[['side', 'liquidity_ref', 'smoothed_size', 'max_local']]
		data = data[data['liquidity_ref'] != 0]
		data = pd.concat([data] + [data[data['max_local'] == True]] * 2).reset_index(drop=True)
